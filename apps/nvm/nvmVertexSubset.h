...
template <class data>
struct nvmVertexSubsetData {
  using S = tuple<uintE, data>;
  using D = tuple<bool, data>;

  // An empty vertex set.
  nvmVertexSubsetData(size_t _n) : n(_n), m(0), d(NULL), s(NULL), isDense(0) { }

  // A vertexSubset from array of vertex indices.
  nvmVertexSubsetData(long _n, long _m, S* indices)
  : n(_n), m(_m), s(indices), d(NULL), isDense(0) { }

  // A vertexSubset from boolean array giving number of true values.
  nvmVertexSubsetData(long _n, long _m, D* _d)
  ...
  nvmVertexSubsetData(long _n, D* _d)
...
template <>
struct nvmVertexSubsetData<pbbs::empty> {
  using S = uintE;

  // An empty vertex set.
  nvmVertexSubsetData<pbbs::empty>(size_t _n) : n(_n), m(0), d(NULL), s(NULL), isDense(0) {}

  // A vertexSubset with a single vertex.
  nvmVertexSubsetData<pbbs::empty>(long _n, uintE v)
  ...
  // A vertexSubset from array of vertex indices.
  nvmVertexSubsetData<pbbs::empty>(long _n, long _m, S* indices)
  : n(_n), m(_m), s(indices), d(NULL), isDense(0) {}

  // A vertexSubset from array of vertex indices.
  nvmVertexSubsetData<pbbs::empty>(long _n, long _m, tuple<uintE, pbbs::empty>* indices)
  : n(_n), m(_m), s((uintE*)indices), d(NULL), isDense(0) {}

  // A vertexSubset from boolean array giving number of true values.
  nvmVertexSubsetData<pbbs::empty>(long _n, long _m, bool* _d)
  : n(_n), m(_m), s(NULL), d(_d), isDense(1)  {}

  // A vertexSubset from boolean array giving number of true values. Calculate
  // number of nonzeros and store in m.
  nvmVertexSubsetData<pbbs::empty>(long _n, bool* _d)
  : n(_n), s(NULL), d(_d), isDense(1) {
  ...
  // A vertexSubset from boolean array giving number of true values. Calculate
  // number of nonzeros and store in m.
 nvmVertexSubsetData<pbbs::empty>(long _n, tuple<bool, pbbs::empty>* _d)
...
using nvmVertexSubset = nvmVertexSubsetData<pbbs::empty>;
