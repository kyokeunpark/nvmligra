...
//input nvmVertexSubset is returned
template <class F>
nvmVertexSubset vertexFilter(nvmVertexSubset V, F filter) {
  ...
  return nvmVertexSubset(n,d_out);
}

template <class F>
nvmVertexSubset vertexFilter2(nvmVertexSubset V, F filter) {
  long n = V.numRows(), m = V.numNonzeros();
  if (m == 0) {
    return nvmVertexSubset(n);
  }
  ...
  out.alloc = false;
  free(bits);
  return nvmVertexSubset(n, out.size(), out.s);
}

template <class data, class F>
nvmVertexSubset vertexFilter2(nvmVertexSubsetData<data> V, F filter) {
  long n = V.numRows(), m = V.numNonzeros();
  if (m == 0) {
    return nvmVertexSubset(n);
  }
