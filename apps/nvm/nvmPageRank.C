...
void Compute(nvmgraph<vertex>& GA, commandLine P) {
  ...
  nvmVertexSubset Frontier(n,n,frontier);
  
  long iter = 0;
  while(iter++ < maxIters) {
    nvmEdgeMap(GA,Frontier,PR_F<vertex>(p_curr,p_next,GA.V),0, no_output);
    nvmVertexMap(Frontier,PR_Vertex_F(p_curr,p_next,damping,n));
    ...
    //reset p_curr
    nvmVertexMap(Frontier,PR_Vertex_Reset(p_curr));
    swap(p_curr,p_next);
  }
